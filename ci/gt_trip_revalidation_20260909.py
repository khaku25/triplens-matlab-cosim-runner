...
def main():
    ...
    try:
        ...
        hp_open = traces['hpTurbineInletValveOpening']
        mp_open = traces['mpTurbineInletValveOpening']

        hp_valve_stayed_native = max(abs(x - 0.8) for x in hp_open) < 1e-6
        mp_valve_stayed_native = max(abs(x - 0.8) for x in mp_open) < 1e-6
        min_change = pct(min_st, pre['stElectricalPower'])
        ...
        report.update({
            ...
            'st_power_min_change_pct_vs_pretrip': min_change,
            ...
            'interpretation_gate': {
                'strong_trip_like_if_st_power_drop_pct_le_minus_70': min_change is not None and min_change <= -70.0,
                'moderate_derating_if_drop_between_20_and_70_pct': min_change is not None and -70.0 < min_change <= -20.0,
                'weak_response_if_drop_gt_minus_20_pct': min_change is not None and min_change > -20.0,
            },
        })
